...
const SocialIconRow = (
    props:{icon_color?:string, color?:string, url:string, fa:string}) => {
    const color = props.color?props.color:"#000" 
    const icon_color = props.icon_color?props.icon_color:"#FFF" 
    return (
        <a href={props.url} className={style.link} rel="noreferrer" target="_blank" style={{backgroundColor:color}}>
            <i className={props.fa} style={{color:icon_color}}/>
        </a>
    );
}

export const SocialIcon = ({link}:{link:LinkObject}) => <SocialIconRow url={link.url} color={link.color} fa={link.icon} />
